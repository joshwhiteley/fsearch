...
fn destination_picker_preserves_query_selection_and_marks_on_cancel() {
    ...
    let move_entry = app
        .menu_entries()
        .iter()
        .position(|entry| *entry == "move marked to…")
        .unwrap();
    ...
    assert!(
        app.menu_entries()
            .iter()
            .all(|entry| !entry.ends_with("marked"))
    );
    assert!(
        app.menu_entries()
            .iter()
            .all(|entry| !entry.contains("marked to"))
    );
...
fn choosing_copy_destination_keeps_marks() {
    ...
    let copy_entry = app
        .menu_entries()
        .iter()
        .position(|entry| *entry == "copy marked to…")
        .unwrap();
...
fn destination_menu_entries_require_visible_marks() {
    ...
    let entries = app.menu_entries();
    assert!(!entries.contains(&"move marked to…"));
    assert!(!entries.contains(&"copy marked to…"));
}
